...
def merge_annotations(vep_file, amp_file, output_file):
    ...
    vep_sample_id = os.path.basename(vep_file).split("_filtered_")[0]
    ...
    vep[['Ref', 'Alt']] = vep['Ref_Alt'].str.split('/', expand=True)
    ...
    vep = vep.merge(cancevar[['Chr', 'Position', 'Ref', 'Alt', 'Classification']],
                    on=['Chr', 'Position', 'Ref', 'Alt'],
                    how='left')

    # Extract the classification part between '#' and 'EVS' using regular expression
    vep['AMP_ACMG_Classification'] = vep['Classification'].str.extract(r'#(.*?)\s*EVS')

    # Drop duplicate rows based on '#Uploaded_variation'
    vep = vep.drop_duplicates(subset=['#Uploaded_variation'])

    # Apply the function to relevant columns to retain the first value from comma-separated entries
    vep["HGVSc_ANNOVAR"] = vep.apply(retain_first_value, column_name="HGVSc_ANNOVAR", axis=1)
    vep["HGVSp_ANNOVAR"] = vep.apply(retain_first_value, column_name="HGVSp_ANNOVAR", axis=1)
